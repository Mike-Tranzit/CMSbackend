...
class Objects
{
    ...
    public function __construct($plate)
    {
        ...
        $this->model = \common\models\nztmodule3\base\Objects::find()->where('`num_auto`=:num_auto and galina=0 and del in(0,2) and is_nkhp in(0,1) and status_m in (1,2,3,4)', [":num_auto" => $this->plate])->orderBy('id DESC')->one();
    }
    ...
    public static function getChangePriorityParam($priority_allow, $action){
        if($action == 'glonass') return $priority_allow;
        return $priority_allow == 1 ? 2: 1;
    }

    public static function changePriority($data) {
        $model = \common\models\nztmodule3\base\Objects::findOne($data['id']);
        if(!$model) throw new HttpException(500,'Запись не найдена');

        if($data['action'] == 'priority'){
            if($data['priority_allow'] == 1) $model->status_person = 2;
            else {
                if($model->glonass == 1) $model->status_person = 1;
                else $model->status_person = 0;
            }

        }else{
            if($data['glonass'] == 1) {
                $model->glonass = 0;
                if( $model->status_person != 2 ) $model->status_person = 0;
            }else {
               if( $model->status_person != 2 ) $model->status_person = 1;
                $model->glonass = 1;
            }

        }

        if($model->save()) return ['id'=>$data['id'], 'priority_allow'=>self::getChangePriorityParam($data['priority_allow'],$data['action']), 'glonass'=>$model->glonass];
        else throw new HttpException(500,'Ошибка сохранения');
    }
}